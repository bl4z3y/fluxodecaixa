...
def rmFeito(conf):
    if (int(dia) >= 28 and int(dia) <= 31) and not conf['resumo_mensal_feito']:
        ...
        if _.lower() == 's':
            ...
            sai = connsql.exec(cursor, f"SELECT SUBTOTAL FROM {TAB}")
            sai = float(sai[0][0])
            ...
            conf['resumo_mensal_feito'] = True
            with open("fdc.ini", "w") as f: f.write(str(conf))
            return True
        elif _.lower() == 'n': return False
...
def main():
    try: 
        ...
        conf = {'resumo_mensal_feito': False}
    
    if dia == 1: conf['resumo_mensal_feito'] = False

    conf['resumo_mensal_feito'] = rmFeito(conf)
    with open("fdc.ini", "w") as f: f.write(str(conf))

    if not conf['resumo_mensal_feito']:
        os.system("clear")
